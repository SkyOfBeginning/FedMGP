...
class Server_DF(object):
    ...
    def fed_avg(self,chosen_clients):
        ...
        # key 和 prompt都要平均
        result_key = deepcopy(prompts[0].prompt_key.state_dict())
        result_prompt = deepcopy(prompts[0].prompt.state_dict())

        for model in prompts[1:]:
            temp_key = model.prompt_key.state_dict()
            temp_prompt = model.prompt.state_dict()

            result_key += temp_key
            result_prompt +=temp_prompt

        result_key = result_key/len(chosen_clients)
        result_prompt = result_prompt/len(chosen_clients)

        self.global_prompt = deepcopy(self.clients[0].global_prompt)
        self.global_prompt.prompt.load_state_dict(result_prompt)
        self.global_prompt.prompt_key.load_state_dict(result_key)
    ...
    def fed_avg_local_head(self,chosen_clients):
        heads = []
        for i in chosen_clients:
            heads.append(self.clients[i].local_head)
        ...
        self.local_head = deepcopy(self.clients[0].global_head)
        self.local_head.load_state_dict(result_head)
        print('聚合head完成')

    def fed_avg_local(self,chosen_clients):
        prompts = []
        for i in chosen_clients:
            prompts.append(self.clients[i].local_prompt)
        ...
        self.local_prompt = deepcopy(self.clients[0].local_prompt)
        self.local_prompt.load_state_dict(result_prompt)
    ...
    def train_clients(self):
        for i in range(self.task_num * self.global_epoch):
            ...
            for j in range(self.client_num):
                self.clients[j].train_only_local(round=i,args=self.args)

            # self.filter(self.threshold)
            self.fed_avg_local([q for q in range(self.client_num)])
            self.fed_avg_local_head([q for q in range(self.client_num)])
            # self.evaluate_global()
            print('聚合完成，以下是精度：')
            for j in range(self.client_num):
                self.clients[j].set_my_local_prompt(self.local_prompt,self.local_head)
        ...
        if len(chosen_clients) == 1:
            ...
            for h in range(self.local_epoch):
                for iteration, (index, x, y) in enumerate(test_loader):
                    ...
                    output = self.vit(x, task_id=round, cls_features=cls_features, train=True,
                                      train_global_prompt=True)

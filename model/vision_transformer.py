...
from local_prompt import Local_Prompt
...
class Attention(nn.Module):
    ...
    def forward(self, x, local_prompt=None):
        B, N, C = x.shape
        ...
        if local_prompt is not None:
            # prefix key, value
            prompt = local_prompt.permute(1, 0, 3, 2, 4).contiguous()  # 2, B, num_heads, prompt_length, C // num_heads
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            top_k=None, batchwise_prompt=False, prompt_key_init='uniform', head_type='token', use_prompt_mask=False,#下面则是我自己加的
    method='l2p',g_prompt_layer_idx=None,e_prompt_layer_idx=None,g_prompt_length=None,):
        """
        Args:
        ...
        # 用作l2p或者delay_fusion里的 global_prompt
        ...
        if self.method=='l2p':
            if prompt_length is not None and pool_size is not None and prompt_pool:
                # 判断prompt长度，池大小，和是否需要prompt_pool（Boolean）
                self.global_prompt = Global_Prompt(length=prompt_length, embed_dim=embed_dim, embedding_key=embedding_key,
                                            prompt_init=prompt_init,
                                            prompt_pool=prompt_pool, prompt_key=prompt_key, pool_size=pool_size,
                                            top_k=top_k, batchwise_prompt=batchwise_prompt,
            ...
            if e_prompt_layer_idx is not None:
                self.e_prompt = Local_Prompt(length=prompt_length, embed_dim=embed_dim, embedding_key=embedding_key,
                                        prompt_init=prompt_init,
                                        prompt_pool=prompt_pool, prompt_key=prompt_key, pool_size=pool_size,
                                        top_k=top_k, batchwise_prompt=batchwise_prompt,
                                        prompt_key_init=prompt_key_init, num_layers=num_e_prompt,
                                        use_prefix_tune_for_e_prompt=True,
                                        num_heads=num_heads, same_key_value=False)
        else:
            # delay 里的
            ...
            if prompt_length is not None and pool_size is not None and prompt_pool:
                ...
                num_e_prompt = len(self.e_prompt_layer_idx) if self.e_prompt_layer_idx is not None else 0
                if e_prompt_layer_idx is not None:
                    self.local_prompt = Local_Prompt(length=prompt_length, embed_dim=embed_dim, embedding_key=embedding_key,
                                            prompt_init=prompt_init,
                                            prompt_pool=prompt_pool, prompt_key=prompt_key, pool_size=pool_size,
                                            top_k=top_k, batchwise_prompt=batchwise_prompt,
                                            prompt_key_init=prompt_key_init, num_layers=num_e_prompt,
       ...
       ###############################################################################
        ...
        self.head = self.head = nn.Sequential(nn.Linear(self.embed_dim, 512),
                                  nn.ReLU(),
                                  nn.Linear(512, 256),
                                  nn.ReLU(),
        ...
        self.head_local = nn.Sequential(nn.Linear(self.embed_dim, 512),
                                  nn.ReLU(),
                                  nn.Linear(512, 256),
                                  nn.ReLU(),
                                  nn.Linear(256, self.num_classes))
    ...
    # 看这里
    def forward_features_l2p(self, x, task_id=-1, cls_features=None, train=False):
        x = self.patch_embed(x) #把图片切成patch

        if self.global_prompt!=None:
            # 用prompt掩码,可以认为每个task对应end-start的prompt
            # 这里是task-specific的prompt吗？？？
            if self.use_prompt_mask and train:
                start = task_id * self.global_prompt.top_k
                end = (task_id + 1) * self.global_prompt.top_k

                single_prompt_mask = torch.arange(start, end).to(x.device)
                print(single_prompt_mask)
                prompt_mask = single_prompt_mask.unsqueeze(0).expand(x.shape[0], -1)
                print(prompt_mask)

                if end > self.global_prompt.pool_size:
                    prompt_mask = None
            else:
                prompt_mask = None
            res = self.global_prompt(x, prompt_mask=prompt_mask, cls_features=cls_features)
            # 获得patch 长度
    ...
    def forward_head_l2p(self, res, pre_logits: bool = False):
        x = res['x']
    ...
    def forward_features_dual(self, x, task_id=-1, cls_features=None, train=False):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            # 这是总的e_prompt, g_prompt在init开始时候已经定义了
    ...
    def forward_head_dual(self, res, pre_logits: bool = False):
        x = res['x']
        if self.class_token and self.head_type == 'token':
            if self.prompt_pool:
    ...
    def forward_features_delay(self,x, task_id=-1,class_id = None, cls_features=None, train=False):
        # 不要改变分类头1的权重
        # with torch.no_grad():
        #     res = self.forward_features_l2p(x,task_id,cls_features,train)
        #     res_head = self.forward_head_l2p(res)
        # x = res['prompted_embedding']
        # new_cls_feature = res['pre_logits']
        x = self.patch_embed(x)
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            if train:
                # 每个task的掩码为[start,end)
                prompt_mask =class_id.unsqueeze(1)
            else:
                prompt_mask = None

            local_prompt_counter = -1
            # 这是总的e_prompt
            res = self.local_prompt(x, prompt_mask=prompt_mask, cls_features=cls_features)
            local_prompt = res['batched_prompt']
            ...
            for i, block in enumerate(self.blocks):
                ...
                if i in self.e_prompt_layer_idx:
                    local_prompt_counter += 1
                    # Prefix tunning, [B, 2, top_k * e_prompt_length, num_heads, embed_dim // num_heads]
                    x = block(x, prompt=local_prompt[local_prompt_counter])
                else:
                    x = block(x)
    ...
    def forward_head_delay(self,res,pre_logits: bool = False):
        x = res['x']
        if self.class_token and self.head_type == 'token':
            if self.prompt_pool:
                x = x[:, self.total_prompt_len]
        ...
        res['logits'] = self.head_local(x)

        return res



    def forward(self, x, task_id=-1,class_id = None, cls_features=None, train=False,train_global_prompt=False):
        res = dict()
        if self.method=='l2p':
            res = self.forward_features_l2p(x, task_id=task_id, cls_features=cls_features, train=train)
            res = self.forward_head_l2p(res)

        if self.method=='dual':
            res = self.forward_features_dual(x, task_id=task_id, cls_features=cls_features, train=train)
            res = self.forward_head_dual(res)

        if self.method=='delay':
            if train_global_prompt:
                res = self.forward_features_l2p(x, task_id=task_id, cls_features=cls_features, train=train)
                res = self.forward_head_l2p(res)
            else:
                res = self.forward_features_delay(x, task_id=task_id,class_id=class_id, cls_features=cls_features, train=train)
                res = self.forward_head_delay(res)
        return res
    ...
    def init_global_prompt(self):
        ...
        self.global_prompt = Global_Prompt(length=self.prompt_length, embed_dim=self.embed_dim, embedding_key=self.embedding_key,
                                           prompt_init=self.prompt_init,
                                           prompt_pool=self.prompt_pool, prompt_key=self.prompt_key, pool_size=self.pool_size,
                                           top_k=self.top_k, batchwise_prompt=self.batchwise_prompt,
    ...
    def init_local_prompt(self):
        num_e_prompt = len(self.e_prompt_layer_idx) if self.e_prompt_layer_idx is not None else 0
        self.local_prompt = Local_Prompt(length=self.prompt_length, embed_dim=self.embed_dim, embedding_key=self.embedding_key,
                     prompt_init=self.prompt_init,
                     prompt_pool=self.prompt_pool, prompt_key=self.prompt_key, pool_size=100,
                     top_k=self.top_k, batchwise_prompt=self.batchwise_prompt,
                     prompt_key_init=self.prompt_key_init, num_layers=num_e_prompt,
                     use_prefix_tune_for_e_prompt=True,
                     num_heads=12, same_key_value=False)

    def get_head(self):
        return self.head,self.head_local

    def set_head(self,head,head_local):
        self.head = deepcopy(head)
        self.head_local = deepcopy(head_local)

    def set_global_head(self,head):
        self.head = deepcopy(head)
